// RTTECPlugin.cpp : アプリケーションのエントリ ポイントを定義します。
//

#include "stdafx.h"
#include "RTTECPlugin.h"
                     ...
                     int       nCmdShow)
{
	...
	LoadString(hInstance, IDC_RTTECPlugin, szWindowClass, MAX_LOADSTRING);
	...
	if (argc < 6) {	// 最後の引数はランチャーからもらう"-run"
	...
	if (0 != _tcsicmp(argv[5], g_szExecutableOption)) {
	...
	g_uNotifyPort = static_cast<USHORT>(_wtoi(argv[3]));
	OutputDebugString(argv[3]);
	g_uRTTPort = static_cast<USHORT>(_wtoi(argv[4]));
	...
	hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_RTTECPlugin));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.hIcon			= LoadIcon(hInstance, MAKEINTRESOURCE(IDI_RTTECPlugin));
	wcex.hCursor		= LoadCursor(NULL, IDC_ARROW);
	wcex.hbrBackground	= (HBRUSH)(COLOR_WINDOW+1);
	wcex.lpszMenuName	= MAKEINTRESOURCE(IDC_RTTECPlugin);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	static RTTECController controller;
